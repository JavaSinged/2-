// 3-5마우스로원선택하기.cpp : 응용 프로그램에 대한 진입점을 정의합니다.
//

#include "stdafx.h"
#include "3-5마우스로원선택하기.h"
...
//
#define BSIZE 50
double LPTS(int x1, int y1, int x2, int y2) {
	double l = (sqrt((float)((x2 - x1)*(x2 - x1) + ((y2 - y1)*(y2 - y1)))));
	return l;
}

BOOL InCircle(int x, int y, int mx, int my) {
	if (LPTS(x, y, mx, my) < BSIZE)return TRUE;
	else return FALSE;
}

LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	static int x, y; //원의 중심 좌표
	static int mx, my;
	static bool Selection; //원 선택유무
	switch (message)
    {
	case WM_CREATE:
		x = 50, y = 50;
